...
with st.sidebar:
    ...
    st.markdown(
        "[🔗 github.com/nimasharifiniko](https://github.com/nimasharifiniko)")
    st.markdown("---")

    st.title("⚙️ Orchestrator Config")

    st.subheader("Model Configuration")
    st.text_input("LLM Model", value=os.getenv(
        "LLM_MODEL", "qwen2.5-coder:7b"), disabled=True)
    ...
    quality_threshold = st.slider(
        ...
        max_value=10,
        value=7,
        help="Minimum Reviewer score required for approval."
    )
...
# --- HEADER ---
st.markdown('<div class="main-title">🤖 Multi-Agent Orchestration System</div>',
            unsafe_allow_html=True)
...
st.markdown('<div class="brand-tag">CRAFTED BY NIMA SHARIFI NIKO</div>',
            unsafe_allow_html=True)
...
topic_input = st.text_input(
    ...
    value=default_topic,
    placeholder="e.g., The Future of Quantum Computing in Security"
)
...
if run_button:
    if not topic_input or not topic_input.strip():
        st.error("⚠️ Please enter a valid topic before running the workflow.")
    else:
        try:
            ...
            # === EXECUTE WORKFLOW WITH ERROR CATCHING ===
            ...
            # Check for catastrophic status
            if state.status == WorkflowStatus.FAILED:
                st.error(
                    f"⚠️ Workflow Execution Failed: {state.error or 'Unexpected system error'}")
            else:
                ...
                if state.draft:
                    ...
                    with writer_output.expander("View Draft Structure", expanded=False):
                        ...
                        st.write(
                            f"**Key Points Count:** {len(state.draft.key_points)}")

                if state.review:
                    reviewer_status.success(
                        f"✅ Status: Evaluated ({state.review.quality_score}/10)")
                    with reviewer_output.expander("View Reviewer Feedback", expanded=False):
                        st.metric(label="Quality Score",
                                  value=f"{state.review.quality_score} / 10")
                        st.write(
                            f"**Approval Status:** {'Approved ✅' if state.review.approved else 'Needs Revision ⚠️'}")
                ...
                if state.iteration > 1:
                    feedback_banner.warning(
                        f"🔁 **Feedback Loop Activated:** Report underwent **{state.iteration} iterations** "
                        f"before meeting the quality threshold ({quality_threshold}/10)."
                    )
                else:
                    feedback_banner.success(
                        f"🎯 **First-Pass Approval:** Draft passed quality threshold ({quality_threshold}/10) "
                        f"on the first iteration!"
                    )
                ...
                if state.final_output:
                    ...
                    st.error("⚠️ No final report output was produced.")

        except Exception as err:
            st.error(
                "⚠️ Unable to complete the workflow. Please make sure your local LLM server (Ollama) is running.")
            st.caption(f"Technical details: {str(err)}")

# --- FOOTER ---
st.markdown(
    '<div class="footer-brand">© 2025 · Multi-Agent Orchestration System · Developed by Nima Sharifi Niko</div>',
    unsafe_allow_html=True
)
